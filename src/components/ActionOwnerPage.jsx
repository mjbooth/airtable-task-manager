...
const ActionOwnerPage = () => {
  ...
  const groupedTasks = useMemo(() => {
    const grouped = clientFilteredTasks.reduce((acc, task) => {
      ...
      if (!acc[assignedOwnerName]) {
        acc[assignedOwnerName] = {};
      }
      if (!acc[assignedOwnerName][clientName]) {
        acc[assignedOwnerName][clientName] = [];
      }
      acc[assignedOwnerName][clientName].push(task);

      return acc;
    }, {});

    // Add pinned clients with no tasks to their assigned owner's column
    pinnedClients.forEach(pinnedClientName => {
      const assignedOwnerName = getClientAssignedOwner(pinnedClientName);
      if (!grouped[assignedOwnerName]) {
        grouped[assignedOwnerName] = {};
      }
      if (!grouped[assignedOwnerName][pinnedClientName]) {
        grouped[assignedOwnerName][pinnedClientName] = [];
      }
    });

    // Sort clients alphabetically within each assigned owner
    Object.keys(grouped).forEach(owner => {
      grouped[owner] = Object.entries(grouped[owner])
  ...
  return (
    <Box width="100%" maxWidth="100%" height="calc(100vh - 104px)">
      <Flex justify="space-between" align="center" mb={4}>
        <Flex gap={6} wrap="nowrap" minWidth="0">
          <FormControl display="flex" alignItems="center" flexShrink={0}>
            <FormLabel htmlFor="show-completed" mb="0" whiteSpace="nowrap">
              Show Closed Tasks
            </FormLabel>
            <Switch
              id="show-completed"
          ...
          <FormControl display="flex" alignItems="center" flexShrink={0}>
            <FormLabel htmlFor="show-closed-clients" mb="0" whiteSpace="nowrap">
              Show Closed Clients
            </FormLabel>
            <Switch
              id="show-closed-clients"
              isChecked={showClosedClients}
              onChange={(e) => setShowClosedClients(e.target.checked)}
              ml={2}
            />
          </FormControl>
        </Flex>
      ...
      >
        <HStack
          ...
          height="100%"
        >
          {sortedGroupedTasks.map(([assignedOwner, clientTasks]) => (
            <Box
              key={assignedOwner}
              width="460px"
              bg="gray.50"
              ...
              flexShrink={0}
            >
              <Heading as="h3" size="md" mb={4} textAlign="left">{assignedOwner}</Heading>
              <VStack spacing={4} align="stretch">
                {Object.entries(clientTasks)
                  .concat(
                    pinnedClients
                      .filter(clientName => {
                        // Only include pinned clients that belong to this owner, don't already have tasks, and respect the closed clients toggle
                        return !clientTasks[clientName] && 
                               getClientAssignedOwner(clientName) === assignedOwner &&
                               (showClosedClients || !isClientClosed(clientName));
                      })
...
export default ActionOwnerPage;